...
// Mirrors interns/InternView.js's consolidation, bucketed for new-grad instead.
const SINCE = [
...
export default function NewGradView() {
  ...
  const load = useCallback(async () => {
    ...
    // etc.) don't surface here as new grad.
    ...
    const results = await Promise.all(
      BOARDS.map(async (b) => {
        ...
        const norm = (data || []).filter(isNewGrad).map((r) => ({
          key: `${b.table}:${r.id}`,
          table: b.table,
          id: r.id,
          company: r.company,
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-4">
        ...
        <span className="ml-auto text-sm text-gray-500">
          {loading ? 'Loading…' : `${visible.length} new grad roles across all boards`}
        </span>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
              ...
              <Fragment key={`day-${bi}`}>
                <tr className="bg-gray-100/70">
                  <td colSpan={10} className="border-y px-3 py-1.5 text-xs font-semibold text-gray-600">
                    📅 {formatDay(batch.ts)} · {batch.rows.length} {batch.rows.length === 1 ? 'new grad role' : 'new grad roles'}
                  </td>
                </tr>
                {batch.rows.map((r) => (
            ...
            {!loading && visible.length === 0 && (
              <tr><td colSpan={10} className="px-3 py-6 text-center text-gray-500">No new grad roles match these filters.</td></tr>
            )}
          </tbody>
        </table>
      </div>
